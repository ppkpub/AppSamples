...
contract GenesisPool is MemeTokenWrapper, Ownable {
    ...
    //constructor()  {
	//	constructor(IERC20(0xBD5C1b5900B8106192214f8E08Ae8D8dDfB08210),IERC20(0x34c685cFEE600445f2Ab06FC8650C7783DECc192));
	//}

    // 0xBD5C1b5900B8106192214f8E08Ae8D8dDfB08210,0x7fc68c65500e8C14fd7b301F2c5CE8f4eD5E80B4
	constructor(IERC20 _MarketTokenAddress,IERC20 _arbTokenAddress,ERC1155Tradable _memesAddress) public MemeTokenWrapper(_MarketTokenAddress) {
	...
	function earned(address account) public view returns (uint256) {
		...
		return
			points[account].add(
				...
				blockTime.sub(lastUpdateTime[account]).div(60).mul(balanceOf(account))
			);
	}

	// stake visibility is public as overriding MemeTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		//require(amount.add(balanceOf(msg.sender)) <= 5, "Cannot stake more than 5 meme");
		require(amount>0, "no meme");
    ...
    //积分兑换仲裁治理和参与代币
	function redeemToMintArbToken(uint256 redeem_point_amount) public updateReward(msg.sender) {
		require(redeem_point_amount> 0, "invalid redeem_point_amount");
		require(points[msg.sender] >=redeem_point_amount, "Not enough points to redeem");
        
		points[msg.sender] = points[msg.sender].sub(redeem_point_amount);
        arbTokenAddress.transfer(msg.sender, redeem_point_amount);
		emit Redeemed(msg.sender, redeem_point_amount);
	}
    
    //积分兑换1155NFT
    function redeemCardNFT(uint256 card) public updateReward(msg.sender) {
